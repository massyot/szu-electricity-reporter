**本项目是基于[另一个项目](https://github.com/ceynri/szu-electricity-reporter)的一点改进**
...
![msg.jpg]([p97mgXj.jpg (534×291) (ax1x.com)](https://s1.ax1x.com/2023/05/24/p97mgXj.jpg))

详细数据：

![detail.jpg]([p97mRns.jpg (687×717) (ax1x.com)](https://s1.ax1x.com/2023/05/24/p97mRns.jpg))
...
| server_chan_key | server 酱的密钥，用于微信提醒(支持设置多个密钥)              | "SCT210954T4Oxxxxxxxxxxxx2qYsBBjZvv" |
| remind_time     | 每日提醒的时间（单位：时，整数，范围 0~23） ps：这个我没用过，可以看看原项目用法。 | 9                                    |
...
3. 注册 Server 酱 账号，获得 SendKey，填入 config.json 的 `server_chan_key`。

4. 运行 `szu-electricity-reporter.exe`，或者用命令行执行 `python main.py` 即可收到微信提醒。

6. 为了避免电脑重启后需要重新手动运行该程序，我们可以将 `szu-electricity-reporter.exe`的快捷方式添加为启动项。

> 这里介绍其中一种添加开启启动项的方法：
>
> 我们找到“启动”文件夹，windows 一般在以下目录：`C:\Users\{你的用户名}\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup`
>
> 将 szu-electricity-reporter.exe 的快捷方式放入其中即可
